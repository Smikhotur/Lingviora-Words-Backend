...
export const emailTokens = sqliteTable(
  ...
  (table) => [
    uniqueIndex("uq_email_tokens_hash").on(table.tokenHash),
    index("idx_email_tokens_user_type").on(table.userId, table.type),
  ],
);
...
export const wordLists = sqliteTable(
  ...
  (table) => [
    index("idx_word_lists_user_updated").on(table.userId, table.updatedAt),
  ],
);
...
export const words = sqliteTable(
  "words",
  {
    ...
    note: text("note"),
    status: text("status", { enum: ["new", "learning", "learned"] })
      .notNull()
      .default("new"),
  ...
  (table) => [
    uniqueIndex("uq_words_list_term_translation").on(
      table.listId,
      table.term,
      table.translation,
    ),
    index("idx_words_list_due_status").on(
      table.listId,
      table.nextReviewAt,
      table.status,
    ),
...
export const pronunciationCache = sqliteTable(
  "pronunciation_cache",
  {
  ...
  (table) => [primaryKey({ columns: [table.language, table.term] })],
);
...
export const learningAttempts = sqliteTable(
  "learning_attempts",
  {
    ...
    mode: text("mode", {
      enum: ["choice", "typing", "sentence", "known"],
    }).notNull(),
    answer: text("answer"),
...
export const emailOutbox = sqliteTable(
  ...
  (table) => [
    index("idx_email_outbox_recipient_created").on(
      table.recipient,
      table.createdAt,
    ),
  ],
