import { z } from "zod";
import { normalizeStudyLanguage } from "./languages";
...
const studyLanguageSchema = z.string().trim().min(2).max(40).transform(normalizeStudyLanguage);
export const listSchema = z.object({ name: z.string().trim().min(1, "Назвіть список").max(80), sourceLanguage: studyLanguageSchema, targetLanguage: studyLanguageSchema });
